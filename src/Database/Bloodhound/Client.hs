...
--   primary key for the document (if not specified, it will be
...
indexDocument :: (ToJSON doc, MonadBH m) => IndexName -> MappingName
                 -> IndexDocumentSettings -> doc -> Maybe DocId -> m Reply
indexDocument (IndexName indexName)
  (MappingName mappingName) cfg document = \case
    Nothing -> bindM2 post (url [indexName, mappingName]) (return body)
    Just (DocId docId) -> bindM2 put (url [indexName, mappingName, docId]) (return body)
  where url paths = addQuery params <$> joinPath paths
...
mkBulkStreamValue :: Text -> IndexName -> MappingName -> Maybe DocId -> Value
mkBulkStreamValue operation (IndexName indexName) (MappingName mappingName) mdocId = object [
  operation .= case mdocId of
    Nothing -> object [
        "_index" .= indexName
      , "_type"  .= mappingName
      ]
    Just (DocId docId) -> object [
        "_index" .= indexName
      , "_type"  .= mappingName
      , "_id"    .= docId
      ]
...
encodeBulkOperation (BulkIndex indexName mappingName mdocId value) = blob
    where metadata = mkBulkStreamValue "index" indexName mappingName mdocId
          blob = encode metadata `mappend` "\n" `mappend` encode value

encodeBulkOperation (BulkCreate indexName mappingName mdocId value) = blob
    where metadata = mkBulkStreamValue "create" indexName mappingName mdocId
          blob = encode metadata `mappend` "\n" `mappend` encode value

encodeBulkOperation (BulkDelete indexName mappingName docId) = blob
    where metadata = mkBulkStreamValue "delete" indexName mappingName $ Just docId
          blob = encode metadata

encodeBulkOperation (BulkUpdate indexName mappingName docId value) = blob
    where metadata = mkBulkStreamValue "update" indexName mappingName $ Just docId
